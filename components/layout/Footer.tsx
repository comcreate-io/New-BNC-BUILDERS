...
export function Footer() {
  ...
  return (
    <footer className="bg-dark text-white">
      {/* Accolades Bar */}
      <div className="border-b border-zinc-800 py-6 sm:py-8">
        <Container>
          <div className="flex flex-wrap justify-center items-center gap-3 sm:gap-6">
            {accolades.slice(0, 6).map((accolade) => (
              <div
                key={accolade.name}
                className="text-zinc-400 text-xs sm:text-sm font-medium px-3 sm:px-4 py-1.5 sm:py-2 border border-zinc-700 rounded opacity-80 hover:opacity-100 hover:border-zinc-500 transition-all"
              >
                {accolade.name}
              </div>
            ))}
        ...
        <Container>
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 sm:gap-8">
            ...
            <div className="col-span-2 md:col-span-4 lg:col-span-2">
              ...
              />
              <p className="text-zinc-400 mb-4 max-w-sm text-sm sm:text-base">
                {company.tagline}
              </p>
              <address className="not-italic text-zinc-400 space-y-2 text-sm sm:text-base">
              ...
              <ul className="space-y-1.5 sm:space-y-2">
                {footerNavigation.company.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-zinc-400 hover:text-gold-light transition-colors text-sm sm:text-base"
                    >
                      {link.label}
                    </Link>
                  </li>
            ...
            <div className="flex items-center gap-4">
              {socialLinks.slice(0, 6).map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  className="text-zinc-400 hover:text-gold-light transition-colors"
